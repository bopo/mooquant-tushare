...
# def is_holiday(date):
#     today = datetime.datetime.strptime(date, '%Y-%m-%d') if isinstance(date, str) else date
#
    ...
    def __wait(self):
        ...
        while not self.__stopped and utcnow() < nextCall:
            ...
            if time_diff < TuSharePollingThread.TUSHARE_INQUERY_PERIOD:
                time.sleep(
                    TuSharePollingThread.TUSHARE_INQUERY_PERIOD -
                    time_diff)
...
class TuShareLiveFeed(barfeed.BaseBarFeed):
    ...
    def __init__(
        ...
        self.__thread = TushareBarFeedThread(
            self.__queue, identifiers, frequency)
    ...
    def getNextBars(self):
        ...
        try:
            eventType, eventData = self.__queue.get(
                True, TuShareLiveFeed.QUEUE_TIMEOUT)
